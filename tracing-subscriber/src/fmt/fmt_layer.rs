use crate::{
    field::RecordFields,
    fmt::{FmtTarget, FormatEvent, FormatFields, MakeWriter, TestWriter, format},
    layer::{self, Context},
...
impl<S, N, E, W> Layer<S, N, E, W> {
    ...
    /// #
    /// info_internal!("This will be logged to stderr");
    /// reload_handle.modify(|layer| *layer.writer_mut() = non_blocking(std::io::stdout()));
    /// info_internal!("This will be logged to stdout");
    /// # }
    /// ```
    ///
    /// [writer]: MakeWriter
    ...
    pub fn with_ansi(self, ansi: bool) -> Self {
        ...
        if ansi {
            const ERROR: &str = "tracing-subscriber: the `ansi` crate feature is required to enable ANSI terminal colors";
...
mod test {
    ...
    fn modify_span_events() {
        ...
        with_default(reload, || {
            ...
            {
                let span2 = tracing::info_span_internal!("span2", x = 100);
                let _e = span2.enter();
            }

            {
                let span3 = tracing::info_span_internal!("span3", x = 42);
                let _e = span3.enter();
